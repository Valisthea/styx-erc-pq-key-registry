...
Contracts that do not implement this EIP are unaffected. Contracts that want PQ key verification can query the registry via `activeKeyFor()` and the optional `IERC8231_OnChainVerify` extension.
