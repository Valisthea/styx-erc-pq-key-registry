...
interface IERCWWWW_Attestation is IERCWWWW {
    ...
    struct Attestation {
        ...
        bytes4 attestationType;  // Type of attestation (ATT_* constants)
        uint256 timestamp;       // Block timestamp when attestation was recorded
        bytes data;              // Attestation-specific data (attestation certificate hash, etc.)
    }
    ...
    ///         The `data` field SHOULD contain a hash of supporting documentation
    ///         (e.g., keccak256 of an HSM certificate or audit report PDF).
    /// @param keyId           Key to attest.
    /// @param attestationType One of the ATT_* constants or a custom bytes4.
